...
# Zoom tour à tour sur les composantes de notre base de données.

## Cement (voir annexe fig 1)
...
## Blast Furnace Slag (voir annexe fig 2)
...
## Superplasticizer (voir annexe fig 5)
...
## Coarse Aggregate (voir annexe fig 6)
...
## Fine Aggregate (voir annexe fig 7)
...
## Concrete Compressive Strength (voir annexe fig 9)
...
# Expliquons la résistance à la compression du béton par nos variables. 
...
## Quantité de ciment et résistance du béton. (voir annexe fig 12)
...
## Additif chimique et eau. (voir annexe fig 13)
...
## L'age, une variable importante. (voir annexe fig 14)
...
## La resistance à la compression du béton peut-elle s'expliquer grâce au laitier de haut fournaux ? (voir fig 15)
...
## Les cendres volantes peuvent-elles expliquer la resistance du béton ? (voir annexe fig 16)
...
## L'eau, un facteur de résistance ? (voir Annexe fig 17)
...
## Superplasticité et Résistance (voir annexe fig 18)
...
##  Granulats grossiers et résistance à la compression (voir annexe fig 19 )
...
## Une relation floue entre les granulats fins et résistance à la compression. ( voir annexe fig 20 )

La présence de granulats fins améliore la compacité du mélange, ce qui peut influencer ses propriétés, notamment la résistance à la compression. Ils remplissent les espaces entre les granulats plus grossiers.

Le graphique des granulats fins ne permet pas de tirer de conclusions claires concernant la relation entre cette variable et la résistance à la compression, aucune tendance évidente n’y apparaît.

```{r, echo = F, eval = F}
test <- cor.test(bdd$fine_aggr, bdd$y_concrete_compresive, method = "kendall")
```

Les résultats du test de corrélation de Kendall indiquent une relation significative entre la quantité de granulats fins et la résistance du béton. Le coefficient $\tau$ étant négatif, cela signifie que lorsque la proportion de granulats fins diminue, la résistance à la compression tend à augmenter. Cette observation suggère qu'un excès de granulats fins pourrait réduire la compacité du mélange.
...
# Exploration des corrélations et des influences combinées
...
### Dim(1-3) : 40.5%  % de la Variance Totale (Voir annexe : fig 21)
...
Cette selection de modèle nous donne le résultat suivant :

`cement + blast_furnace + fly_ash + water + super_plast + age`
...
On peut alors essayer de trier les modèle sur un dernier critaire comme par exemple le critère du $R^2$

```{r, echo = F}
# Récupération des modèles candidats
modeles <- as.data.frame(weightable(select_aic))

# Vérifie le nom de la colonne contenant les modèles


# Supposons que la colonne soit nommée 'model'
# Sinon, remplace 'model' par le vrai nom affiché par head(modeles)

# Initialiser un vecteur pour stocker les R²
r2_values <- numeric(nrow(modeles))

# Calculer le R² pour chaque modèle correctement :
for(i in 1:nrow(modeles)){
  
  # Construire correctement la formule
  variables <- modeles[i, "model"]   # attention à bien préciser le nom exact de la colonne
  formule <- as.formula(variables)
  
  # Ajuster le modèle et calculer le R²
  fit <- lm(formule, data = bdd)
  r2_values[i] <- summary(fit)$r.squared
}

# Ajouter le R² au tableau pour comparaison
modeles$R2 <- r2_values

# Trier selon le R² décroissant pour trouver le meilleur modèle selon ce critère
modeles <- modeles[order(-modeles$R2), ]

# Afficher clairement les modèles triés par R²

```

Ce filtre du R2 a ajouté la variable `coarse` qui avait disparu lors de l'utilisation des deux critères précédents. Aisni le modèle optimal en utilisant le critère R2 est  :

`cement + blast_furnace + fly_ash + water + super_plast + age + coarse`

**Erreur empirique**

On va appliquer ici les méthode pour estimer l'erreur empirique d'un modèle : on regarde de modele complet et le modèle optimale selon BIC à chaque fois
...
n <- nrow(bdd) #nombre d'observations
...
ensemble_apprentissage <- bdd[indices, ] #la partie de la base de données `Auto` qui va servir pour l'apprentissage


ensemble_validation <- bdd[ - indices, ] #la base de validation



modele_1 <- glm(y_concrete_compresive ~ ., data = ensemble_apprentissage)

modele_2 <- glm(y_concrete_compresive ~ cement + blast_furnace + fly_ash + water + super_plast + age, data = ensemble_apprentissage) 

#estimation de l'erreur du modele1
#3
valeurs_predites_1 <- predict(object = modele_1, newdata = ensemble_validation)
valeurs_predites_2 <- predict(object = modele_2, newdata = ensemble_validation) 

estimation_erreur_modele_1<- mean((ensemble_validation$y_concrete_compresive- valeurs_predites_1)^2)  
estimation_erreur_modele_2<- mean((ensemble_validation$y_concrete_compresive- valeurs_predites_2)^2) 
estimation_erreur_modele_1
estimation_erreur_modele_2
```

On voit ici que piur la méthode d'apprentissage validation, le modèle pris n'influe pas sur la valeur de l'erreur.
...
mse_moyen <- mean(mse_folds)

cat("Erreur estimée par validation croisée (", K, "-fold) :", mse_moyen, "\n")

```

On retrouve une erreur plus faible mais sans différence abérente des résultats ce qui indique une stabilité du modèle de regression.
...
### Fig 1 : Histogramme pour la variable `Cement`
...
###  Fig 2 : Histogramme pour la variable `Blast Furnace Slag`
...
###  Fig 3 :Histogramme pour la variable `Fly Ash`
...
###  Fig 4: Histogramme pour la variable `Water`
...
###  Fig 5 : Histogramme pour la variable `Superplasticizer`
...
###  Fig 6 : Histogramme pour la variable `Coarse Aggregate`
...
###  Fig 7 : Histogramme pour la variable `Fine Aggregate`
...
###  Fig 8 : Histogramme pour la variable `Age`
...
###  Fig 9 : Histogramme pour la variable `Concrete Compressive Strength`
...
### Fig 12 : Nuage des points entre le ciment et la resistance à la compression
...
### Fig 13 : Relation entre l'eau et Super plasticizer
...
### Fig 14 : Relation entre l'age et la resistance
...
### Fig 15 : Relation entre laitier haut fourneau et Résistance à la Compression
...
### Fig 16 : Relation entre Cendres volantes et Résistance à la Compression
...
### Fig 17 : Relation entre eau et Résistance à la Compression
...
### Fig 18 : Relation entre additif chimique et Résistance à la Compression
...
### Fig 19 : Relation entre granulats grossiers et Résistance à la Compression
...
### Fig 21 : PCA sur  la dimension 1-3
...
### Fig 22 : PCA sur  la dimension 2-3
